  ...
  <label>État
    <select name="condition">
      <?php foreach (['A','B','C','D'] as $c): ?>
        <option <?= ($product['condition']==$c?'selected':'') ?>><?= $c ?></option>
      <?php endforeach; ?>
    </select>
  </label>
  <label>Format
    <select name="format">
      <?php foreach (['35mm','120','APS','Autre'] as $f): ?>
        <option <?= ($product['format']==$f?'selected':'') ?>><?= $f ?></option>
      <?php endforeach; ?>
    </select>
  </label>
  <label>Prix CHF <input name="price_chf" type="number" step="0.05" min="0" value="<?= htmlspecialchars($product['price_chf']) ?>" required></label>
  <label>Stock <input name="stock_qty" type="number" min="0" value="<?= (int)$product['stock_qty'] ?>"></label>
  <label><input type="checkbox" name="is_active" <?= ($product['is_active']?'checked':'') ?>> Actif</label>
